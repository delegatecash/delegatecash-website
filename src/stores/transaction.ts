...
function createTransactionStore() {
  ...
  return {
    subscribe,
    setData: (title, message, showModal = false, tx = null) =>
      update(obj => {
        return {
          ...obj,
          title,
    ...
    setTransactionUrl: (transactionUrl: string) =>
      update(obj => {
        return {
          ...obj,
          transactionUrl,
...
export const setTransactionError = err => {
  if (err.code === 4001) transaction.setMessage('User denied the transaction');
  ...
  else transaction.setMessage('Wallet not connected. Please connect your wallet.');
};
